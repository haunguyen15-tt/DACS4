...
class ChatsProvider with ChangeNotifier {
  ...
  Future<void> getAllChats() async {
    ...
    print("CHAT DEBUG" + response['chats'].toString());
    // tempChatContacts = ;
    ...
    print("ALL CHATS");
    print(tempChatContacts[0].toJson());
    notifyListeners();
  ...
  Future _setAllChats(var newChats) async {
    ...
    newChats.forEach((newChat) async {
      await databaseHelper.insert({
        ...
        'message': decrypt(newChat['message'],
            EncryptionHelper.convertStringToPrivateKey(user['privateKey'])),
  ...
  void initSocket(id) {
    ...
    socketIO.subscribe('receive_call', (jsonData) async {
      Map<String, dynamic> data = json.decode(jsonData);
      var token = await SharedPreferencesHelper.getAgoraToken();
      var channelName = await SharedPreferencesHelper.getAgoraChannelName();
      print("CALL DATA");
      print(data);
  ...
  void deleteSelectedUserChats(selectedUserId) {
    _allChats.removeWhere((chat) {
      return (chat['from']['_id'] == selectedUserId ||
          chat['to']['_id'] == selectedUserId);
    });
