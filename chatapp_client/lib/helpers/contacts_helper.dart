    ...
    for (int i = 0; i < contactsiterlist.length; i++) {
      ...
      if (contactsiterlist[i].phones.toList().length > 0) {
        ContactModel contact = new ContactModel(
            name: contactsiterlist[i].displayName,
            number: contactsiterlist[i].phones.toList()[0].value);
        contactslist.add(contact.toJson());
      }
    }
    // List<dynamic> verifiedContacts = await ContactsApi.getVerifiedContacts(
    //     {'contactlist': contactslist}, token);
