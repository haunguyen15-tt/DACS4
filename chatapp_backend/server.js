...
cron.schedule('* * * * *', function () {
  var dateTime = moment.tz(Date.now(), 'Asia/Ho_Chi_Minh');
...
io.on('connection', (userSocket) => {
  ...
  userSocket.on('start_call', async (data) => {
    console.log('IT WORKS START CALL');
    console.log(data);
    ...
    await newCall
      ...
      .catch((err) => console.log(err));
    // const agoraTokenResponse = await axios(
    //   `http://192.168.1.186:8000/access-token?channelName=${newCall._id}`
    // );
    // console.log('AGORAAAA 111' + newCall._id);
    // console.log('AGORAAAA' + agoraTokenResponse['data']['token']);
    userSocket.to(data.receiverId).emit('receive_call', data);
  });
