...
router.post('/getotp', async (req, res) => {
  ...
  if (
    ...
    else res.status(500).json({ message: 'Database error', type: 'error' });
...
async function checkOTP(otp, email_id) {
  var result = true;
  try {
    var id = await OTP.findOne(
      { email: email_id, otp: otp },
...
router.post('/getverifiedcontacts', login_required, async (req, res) => {
  ...
  //   // req.body['contactlist'][i]['number'] = req.body['contactlist'][i]['number'];
  //   // req.body['contactlist'][i]['number'] = req.body['contactlist'][i]['number'];
  ...
  const verifiedContacts = await User.find({ _id: { $ne: req.user._id } });
