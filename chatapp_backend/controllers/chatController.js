...
router.get('/getAllChats', login_required, async (req, res) => {
  console.log('Get all chats=', req.user._id);
...
router.get('/getNewChats', login_required, async (req, res) => {
  ...
  Chat.find({ $and: [{ to: req.user._id }, { isStored: false }] })
    ...
    .then((chats) => {
      ...
      Chat.updateMany(
        { $and: [{ to: req.user._id }, { isStored: false }] },
        { isStored: true },
        function (err, doc) {
          if (err) {
            console.log(err);
...
router.post('/setSeen', login_required, async (req, res) => {
  ...
  Chat.updateMany(
    { $and: [{ to: req.user._id }, { from: req.body._id }] },
    { seen: true },
    function (err, doc) {
      if (err) {
        console.log(err);
...
router.get('/getSelectedUserChat', login_required, async (req, res) => {
  ...
  Chat.find({
    $or: [
      { $and: [{ from: req.user._id }, { to: req.body._id }] },
      { $and: [{ to: req.user._id }, { from: req.body._id }] },
    ],
  })
...
exports.chatpost = async (parameters) => {
  ...
  const result = await axios.post(parameters.url, options);
  console.log('HEY');
